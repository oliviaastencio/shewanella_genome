...
while read genome 
	...
	cat $output/$genome.csv/$genome'.csv_COG_list' | cut -f 1,3 | sed s'/LOCUS_//g' > $output/$genome.csv/$genome'.csv_LOCUS_category' 
...
### create the GI list
	...
	do 
		...
		grep "GI"  $output/$genome.csv/$genome.csv'_LOCUS_list' | sed s'/LOCUS_//g' >  $output/$genome.csv/$genome.csv'_GI_LOCUS_list' 
	# 	while read COG  
	# 	do 
	...
	rm -r functional_enrichment
	. ~soft_bio_267/initializes/init_degenes_hunter
	clusters_to_enrichment.R -i $genome.csv'_GI_LOCUS_list' --custom $output/$genome.csv/$genome'.csv_LOCUS_category'
	cd ../
...
cds=`cat $genome_analysis_path/dfast_0000/genome_annotation/results_dfast_parser/"$accession".fna_CDSs`
	...
	do
		...
		awk '{ print $1 "\t" $2 "\t" '"$cds"'}' $output/$accession.fna.csv/category_table.txt | awk '{ print $1 "\t" $2 "\t" $3 "\t" $2*100/$3}' | grep -v "categories" | cut -f 1,2 > $output/$accession.fna.csv/category_table
		awk '{ print $1 "\t" $2 "\t" '"$cds"'}' $output/$accession.fna.csv/category_table.txt | awk '{ print $1 "\t" $2 "\t" $3 "\t" $2*100/$3}' | grep -v "categories" | cut -f 1,4 > $output/$accession.fna.csv/category_table_relative
		sed -i "1i categories\t$name" $output/$accession.fna.csv/category_table
		sed -i "1i categories\t$name" $output/$accession.fna.csv/category_table_relative
	done < $output/$accession.fna.csv/$accession.fna.csv_locus
	sed -i "1i $name" $output/$accession.fna.csv/Total_genomic_island 
	cat $output/$accession.fna.csv/$accession.fna'.csv_COG_list' | cut -f 1,3 | sed s'/LOCUS_//g' >> $output/$accession.fna.csv/$accession.fna'.csv_LOCUS_category'
	...
	do 
		...
		cat $output/$accession.fna.csv/GI/$GI'_GI_LOCUS' | tr '\n' '\t' >> $output/$accession.fna.csv/GI/$GI'_GI_LOCUS_list'
		cat $output/$accession.fna.csv/GI/*_GI_LOCUS_list | sed s'/GI/\nGI/g' > $output/$accession.fna.csv/$accession.fna.csv'_LOCUS_list'
		grep "GI" $output/$accession.fna.csv/$accession.fna.csv'_LOCUS_list' | sed s'/LOCUS_//g' > $output/$accession.fna.csv/$accession.fna.csv'_GI_LOCUS_list'
		# while read COG 
		# do 
	...
	cd $output/$accession.fna.csv/
	rm -r functional_enrichment
	. ~soft_bio_267/initializes/init_degenes_hunter
	clusters_to_enrichment.R -i $accession.fna.csv'_GI_LOCUS_list' --custom $accession.fna'.csv_LOCUS_category'
	cd ../
...
merge_tabular.rb $output/*/*category_table_relative | sed s'/.fasta//g' | sed s'/e_Shewanella_putrefaciens_//g' | sed s'/e_//g' | sed s'/_micro12//g' | sed s'/_micro13//g' | sed s'/_micro9//g' | sed s'/_micro22//g' | sed s'/_micro1//g' | sed s'/_1//g'> $output/Total_category_relative
