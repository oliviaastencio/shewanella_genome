...
do 
	cds=`cat $output/"$genome"_CDSs`
 	...
 	cat $output'/'$genome'_cog_table_relative.txt' | cut -f 1,2 > $output/$genome'_cog_table'
 	cat $output'/'$genome'_cog_table_relative.txt' | cut -f 1,4 > $output/$genome'_cog_table_relative'
 	sed -i "1i category\t$genome" $output/$genome'_cog_table'
	sed -i "1i category\t$genome" $output/$genome'_cog_table_relative'
...
while read genome 
   do 
    ...
    cds=`cat $output/"$accession".fna_CDSs`
  	...
  	cat $output/$accession'.fna_cog_table_relative.txt' | cut -f 1,2 > $output/$accession'.fna_cog_table'
  	cat $output/$accession'.fna_cog_table_relative.txt' | cut -f 1,4 > $output/$accession'.fna_cog_table_relative'
  	sed -i "1i category\t$name" $output/$accession'.fna_cog_table'
  	sed -i "1i category\t$name" $output/$accession'.fna_cog_table_relative'
...
#relative data 
merge_tabular.rb $output/*cog_table_relative > $output/Total_table_relative.txt
grep "category" $output/Total_table_relative.txt > $output/category_name_relative
grep -v "category" $output/Total_table_relative.txt > $output/COG_annotation_value_relative
merge_tabular.rb  $data_path/COG_categories_all $output/COG_annotation_value_relative | cut -f 2-133 > $output/COG_annotation_complete_relative
cat $output/category_name $output/COG_annotation_complete_relative | sed s'/.fasta//g' | sed s'/e_Shewanella_putrefaciens_//g' | sed s'/e_//g' | sed s'/_micro12//g' | sed s'/_micro13//g' | sed s'/_micro9//g' | sed s'/_micro22//g' | sed s'/_micro1//g' | sed s'/_1//g' > $output/COG_annotation_relative